...
class InteractiveDemo {
  ...
  async createTerminal(shell, cwd) {
  ...
  async listTerminals() {
    ...
    result.terminals.forEach((terminal) => {
      const current =
        terminal.id === this.currentTerminalId ? " (current)" : "";
      console.log(
        `  ${terminal.id.substring(0, 8)}... - ${terminal.status} - PID: ${terminal.pid}${current}`,
      );
    });
  }
  async selectTerminal(terminalId) {
    ...
    const terminal = result.terminals.find(
      (t) => t.id === terminalId || t.id.startsWith(terminalId),
    );
    if (!terminal) {
  ...
  async readOutput(since, maxLines) {
    if (!this.currentTerminalId) {
      console.log('No terminal selected. Use "select <id>" first.');
      return;
    }
    ...
    console.log("--- End Output ---");
    console.log(
      `Total lines: ${result.totalLines}, Has more: ${result.hasMore}`,
    );
  }
